...
vec2 frand(ivec2 n)
{
	return vec2(ihash(n)) / 2147483647.0;
}

vec3 frand(ivec3 n)
{
	return vec3(ihash(n)) / 2147483647.0;
}
...
vec3 GradientNoise2D( in vec2 p )
{
    vec2 i = floor( p );
    vec2 f = fract( p );
...
//For FBM example
float fbm2D( in vec2 x, in float H ,int numOctaves)//H=0.5
{    
    float G = exp2(-H);
    float f = 1.0;
    float a = 1.0;
...
float fbm3D( in vec3 x, in float H ,int numOctaves)//H=0.5
{    
    float G = exp2(-H);
    float f = 1.0;
    float a = 1.0;
...
vec3 Curl3D(vec3 uv)
{
	...
	float n1, n2, a, b;
	vec3 noise;
    vec3 phi=GradientNoise3D(uv).yzw;
    vec3 phix=GradientNoise3D(uv+eps.xyz).yzw;
    vec3 phiy=GradientNoise3D(uv+eps.yxz).yzw;
    vec3 phiz=GradientNoise3D(uv+eps.yzx).yzw;
    noise.x=(phiy.z-phi.z)-(phiz.y-phi.y);
    noise.y=(phiz.x-phi.x)-(phix.z-phi.z);
    noise.z=(phix.y-phi.y)-(phiy.x-phi.x);
	// n1 = GradientNoise3D(uv + eps.yxz).x;
	// n2 = GradientNoise3D(uv - eps.yxz).x;
	// a = (n1 - n2) / (2. * eps.x);
	// n1 = GradientNoise3D(uv + eps.yzx).x;
	// n2 = GradientNoise3D(uv - eps.yzx).x;
	// b = (n1 - n2) / (2. * eps.x);
	// noise.x = a - b;
	...
	// n2 = GradientNoise3D(uv - eps.yzx).x;
	// a = (n1 - n2) / (2. * eps.x);
	// n1 = GradientNoise3D(uv + eps.xyz).x;
	// n2 = GradientNoise3D(uv - eps.xyz).x;
	// b = (n1 - n2) / (2. * eps.x);
	// noise.y = a - b;
