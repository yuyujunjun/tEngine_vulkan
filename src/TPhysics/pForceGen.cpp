...
namespace tEngine {
	...
	void Gravity::updateForce(EcsManager* scene, real duration) {
		for (auto entity : SceneView<RigidBody>(*scene)) {
			auto body = scene->GetComponent<RigidBody>(entity);
			if (!body->hasFiniteMass())continue;
			body->addForce(gravity * body->getMass());
		}
	}

	void SpringSystem::updateForce(EcsManager* scene, real duration) {
		for (auto entity : SceneView<Spring>(*scene)) {
			auto spring = scene->GetComponent<Spring>(entity);
			Transform* t0 = nullptr;
			Transform* t1 = nullptr;
			Vector3 pos0, pos1;
			if (spring->connectedBody[0] != -1) {
				t0 = scene->GetComponent<Transform>(spring->connectedBody[0]);
			...
			if (spring->connectedBody[1] != -1) {
				t1 = scene->GetComponent<Transform>(spring->connectedBody[1]);
			...
			force *= -magnitude;
			if (t0) {
				scene->GetComponent<RigidBody>(spring->connectedBody[0])->addForceAtPoint(t0, force, pos0);
			}
			if (t1) {
				scene->GetComponent<RigidBody>(spring->connectedBody[1])->addForceAtPoint(t1, -force, pos1);
			}
		}
	}
	//void Aero::updateForce(EcsManager* scene, real duration) {
	//	for(auto )
