...
void Camera::lookAt(glm::vec3 position) {
    ...
    _verticalAngle = (asinf(-direction.y));
    _horizontalAngle = -(atan2f(-direction.x, -direction.z));
...
glm::vec3 Camera::right() const {
	...
	if(1-glm::dot(forward(),tempup)<0.1)tempup=glm::vec3(0,1,0.1);

	glm::vec3 right=glm::normalize(glm::cross(forward(),tempup));
