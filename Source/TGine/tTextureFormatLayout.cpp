...
#undef astc_fmt
	...
	void TextureFormatLayout::build_buffer_image_copies(std::vector<vk::BufferImageCopy>& copies) const
	{
		...
		for (unsigned level = 0; level < mip_levels; level++)
		{
			...
			blit.imageSubresource.aspectMask = (vk::ImageAspectFlagBits)format_to_aspect_mask(format);
