...
namespace tEngine {
	...
	void CommandBuffer::bindDescriptorSet(vk::PipelineBindPoint bindingPoint, const PipelineLayoutHandle& pipelineLayout, uint32_t firstSet, const std::vector<DescriptorSetHandle>& sets,
		const std::vector<uint32_t>& dynamicOffsets)
	{
		...
		for (const auto& set : sets) {
			bool check = false;
			for (const auto& buf : set->getResource()) {
				if (buf.buffer) {
					_objectReferences.emplace_back(buf.buffer);
					check = true;
				}
				else if (buf.image) {
					_objectReferences.emplace_back(buf.image);
					check = true;
				}
			}
	...
	void CommandBuffer::bindPipeline(const PipelineHandle& pipeline) {
